...
- [x] **2. Eliminar variables `APP_AUTH_OAUTH_*` y código muerto asociado** en `.env.example`, `AuthenticationSettings`, validators de producción. Auditar si hay referencias en docs.

- [x] **3. Eliminar adaptador SMTP** (`src/features/email/adapters/outbound/smtp/`), sus settings, env vars `APP_EMAIL_SMTP_*`, tests, contract tests parametrizados, servicio `mailpit` en `docker-compose`, mención en docs.
...
- [x] **7. Corregir la redacción del "stub S3"** (drift-fix, decisión Option A 2026-05-16). La premisa original era falsa: nunca hubo un stub S3 que levantara `NotImplementedError`. `S3FileStorageAdapter` es un adaptador `boto3` real y se **retiene** (es exactamente lo que pide el paso 24; S3 no está en la lista de remoción SMTP/Resend/arq/SpiceDB). El paso 7 corrigió la redacción obsoleta que llamaba "stub" al adaptador real en los docs.

- [x] **8. Actualizar `docs/api.md`**: borrar todas las referencias a endpoints Kanban (`/api/boards`, `/api/columns`, `/api/cards`). No existen en el código.
