...
#### Scenario: Router dependency contract remains handler-based
- **WHEN** route dependencies are declared for Kanban operations
- **THEN** routes SHALL depend on handler providers (`command` / `query`) rather than repository implementations
