...
The system MUST treat the architecture conformance test suite as the single source of truth for hexagonal-architecture compliance. The skill `fastapi-hexagonal-architecture` review checklist MUST be encoded as machine-verifiable tests, and a passing suite MUST imply conformance.
...
#### Scenario: Skill checklist coverage is complete
- **WHEN** the conformance suite is enumerated
- **THEN** every item in the `Review Checklist` section of the `fastapi-hexagonal-architecture` skill is covered by at least one test, and each test docstring cites the specific checklist item it enforces
...
The conformance suite MUST flag the application-layer anti-patterns listed in the `fastapi-hexagonal-architecture` skill, in particular generic service objects with too many methods and anemic pass-through use cases.
...
- **THEN** the conformance suite fails citing Anti-Pattern 6
...
Each architecture test failure MUST surface a diagnostic that names the specific skill section or checklist line being enforced so that developers can locate normative guidance without further questions.

#### Scenario: Failure message cites checklist item
- **WHEN** the conformance suite fails any assertion
- **THEN** the assertion message contains the substring `fastapi-hexagonal-architecture:` followed by either a section heading or a checklist line excerpt
