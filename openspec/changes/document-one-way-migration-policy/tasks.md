...
- [ ] 1.1 Add a "Migration policy" section to `docs/operations.md` covering:
...
- [ ] 2.1 Edit `alembic/versions/20260513_0010_drop_users_password_hash.py` (lines 34-47): replace the `op.add_column(... server_default="")` body — including the misleading comment "Restore the column with a server-side empty default so existing rows satisfy the NOT NULL constraint" — with:
  ```python
  def downgrade() -> None:
...
- [ ] 3.1 Implement `tests/quality/test_migration_policy.py`:
...
- [ ] 4.1 Add a `make migrations-check` target running `uv run pytest tests/quality/test_migration_policy.py -q`.
- [ ] 4.2 Update the `make ci` recipe so `migrations-check` runs alongside `quality` and `cov`.
...
- [ ] 5.1 Unit: synthetic migration with `op.drop_column("foo", "bar")` + `raise NotImplementedError(...)` → `scan` returns `[]`.
- [ ] 5.2 Unit: synthetic migration with `op.drop_column("foo", "bar")` + `pass` → `scan` returns one violation naming the file and line.
- [ ] 5.3 Unit: synthetic migration with `op.drop_index("idx_foo")  # allow: destructive` + non-raising downgrade → `scan` returns `[]`.
- [ ] 5.4 Unit: synthetic migration with `op.execute("DROP TABLE foo")` + non-raising downgrade → `scan` returns one violation.
- [ ] 5.5 Unit: synthetic migration with `op.execute("alter table foo drop column bar")` (lower case) + non-raising downgrade → `scan` returns one violation (regex/uppercasing works).

## 6. Wrap-up

- [ ] 6.1 `make ci` green (including `make migrations-check`).
