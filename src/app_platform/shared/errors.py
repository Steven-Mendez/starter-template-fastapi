...
Concrete subclasses MUST be picklable so they round-trip across the arq Redis
boundary; if a subclass needs non-positional constructor arguments, it MUST
implement ``__reduce__`` to satisfy ``Exception.__reduce__``'s positional-only
default.
"""
