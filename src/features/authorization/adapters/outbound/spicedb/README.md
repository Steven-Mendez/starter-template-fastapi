...
3. Update `build_authorization_container` to construct
   `SpiceDBAuthorizationAdapter` instead of
   `SQLModelAuthorizationAdapter` (see
   `src/features/authorization/composition/container.py`). The
   application layer does not change.
