...
def test_downgrade_one_recreates_rbac_tables(
    postgres_auth_repository: SQLModelAuthRepository,
    alembic_config: Config,
) -> None:
    """Downgrading the rebac revision restores empty RBAC tables."""
    engine = postgres_auth_repository.engine
    with engine.begin() as conn:
        conn.execute(text("DROP SCHEMA public CASCADE"))
        conn.execute(text("CREATE SCHEMA public"))
...
def test_round_trip_converges_on_the_same_schema(
    ...
    """upgrade -> downgrade -1 -> upgrade SHALL leave the schema unchanged."""
    engine = postgres_auth_repository.engine
    with engine.begin() as conn:
        conn.execute(text("DROP SCHEMA public CASCADE"))
        conn.execute(text("CREATE SCHEMA public"))
